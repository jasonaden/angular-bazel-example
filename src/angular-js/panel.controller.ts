...
import {PANEL_DIALOG_TEMPLATE} from './panel-dialog.template';

const angular = getAngularJSGlobal();

export function AnimationCtrl($mdPanel) {
  this._mdPanel = $mdPanel;
  this.openFrom = 'button';
  this.closeTo = 'button';
  this.animationType = 'scale';
  this.duration = 300;
  this.separateDurations = {open: this.duration, close: this.duration};
}

AnimationCtrl.prototype.showDialog = function() {
  var position = this._mdPanel.newPanelPosition().absolute().right().top();
  ...
  switch (this.animationType) {
    case 'custom':
      animation.withAnimation(
          {open: 'demo-dialog-custom-animation-open', close: 'demo-dialog-custom-animation-close'});
      break;
    case 'slide':
  ...
  var config = {
    ...
    controllerAs: 'ctrl',
    template: PANEL_DIALOG_TEMPLATE,
...
export function DialogCtrl(mdPanelRef) {
